...
;;Fonction qui détermine si un fait appartient bien à la base de fait
...
;;Fonction qui donne les règles candidates du but mis en argument
(defun regles_candidates (but bdr)
  (if bdr
  	(let* ((conclusion (cclRegle (car bdr)))
    	...
    	(if
      	(and
      	(eq attribut (cadr but))  ;; on vérifie si l'attribut est le même
...
(defun verifier_ou (but bdF bdR &optional (i 0)) ;; on regarde si le but est verifiee par les faits de la BDF
  ...
  (if (appartient but bdF) ;; si le fait appartient à la base de fait, il est prouvé
  	(progn
    	;;(format t "~V@t But : ~A proof ~%" i but)
    	T)
	;; sinon, on regarde l'ensemble de ces règles candidates
	(let ((regles (reverse (regles_candidates but bdR))) (ok nil))
 	;;pour chaque de ses règles candidates et tant que le fait n'a pas été prouvé
  	(while (and regles (not ok))
   	;;(format t "~% ~V@t VERIFIE_OU ~A Regles ~s :  ~A ~%" i but (numRegle (car regles)) (car regles))
   	;;on vérifie la règle : tous les éléments de la prémisse
   	(setq ok (verifier_et (pop regles) bdF bdR i)))
 	ok)
	))

(defun verifier_et (regle bdF bdR i) ;; on regarde si tous les éléments de la prémisse sont vérifiés
  (let ((ok t) (premisses (premisseRegle regle)))
	;; pour chaque élement de la premisse et tant que les éléments sont vérifiés
  ...
  (format t "~%------------------Vous avez lance le chainage-arriere----------------------")
    ...
    (while (or (not (numberp age)) (<= age 0))
        ...
        (while (and (not (equal rep 'BON)) (not (equal rep 'MOYEN)) (not (equal rep 'MAUVAIS)))
           (format t "~%Quel est votre niveau de santé ? MAUVAIS, MOYEN, BON : ")
           (setq rep (read))
           (format t "~%"))
